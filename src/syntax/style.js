function findStyleErrors(text) {
  ...
  let state = 0
  const messages = []
  // const tags = new Set(); // Currently unused
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    for (let i = 0; i < mlines.length; i++) {
      ...
      if (
        (match = mlines[i].match(/https?:\/\/streamable.com\/[a-z0-9]{6,}/))
      ) {
        results.push({
          line: message.firstline + i,
          type: 'error',
      ...
      if (
        (match = mlines[i].match(
          /^(?!.*(?:url|value|title|description|name))(?=.*https:\/\/img\.pvme\.io\/images\/)(?!.*<.*https:\/\/img\.pvme\.io\/images\/.*>).*/i
        ))
      ) {
        results.push({
          line: message.firstline + i,
