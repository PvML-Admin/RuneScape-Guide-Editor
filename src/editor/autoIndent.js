        ...
        if (startIndent === nextIndent && !isNaN(nextNumber)) {
          ...
          cm.replaceRange(
            nextLine.replace(
              listRE,
              nextIndent +
                String.fromCharCode(itemNumber) +
                nextItem[4] +
                nextItem[5]
            ),
            {
  ...
  CodeMirror.commands.autoIndentMarkdownList = function (cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (unorderedListRE.test(match[2])) {
        cm.replaceRange(
          getBulletType(pos.ch + 2),
          { line: pos.line, ch: pos.ch + 2 },
          { line: pos.line, ch: pos.ch + 3 }
        )
        lastIndentCharacter = false
      } else {
        cm.replaceRange(
          getAlphaNumericalType(pos.ch + 1),
      ...
      if (pos.ch >= 4) {
        ...
        if (unorderedListRE.test(match[2])) {
          if (lastIndentCharacter)
            cm.replaceRange(
              getAlphaNumericalType(pos.ch - 6),
              { line: pos.line, ch: pos.ch - 6 },
              { line: pos.line, ch: pos.ch - 5 }
            )
          else
            cm.replaceRange(
              getBulletType(pos.ch - 6),
              { line: pos.line, ch: pos.ch - 6 },
              { line: pos.line, ch: pos.ch - 5 }
            )
        } else
          cm.replaceRange(
            getAlphaNumericalType(pos.ch - 7),
            { line: pos.line, ch: pos.ch - 7 },
            { line: pos.line, ch: pos.ch - 5 }
          )
      }
    }
