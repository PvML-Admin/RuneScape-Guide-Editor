...
function formatSpecialCharacters(cm) {
  ...
  for (const result of results.reverse()) {
    ...
    if (specialCharacter) {
      ...
      cm.replaceRange(specialCharacter, cursor, {
        line: cursor.line,
...
function formatChannels(cm) {
  ...
  for (const result of results.reverse()) {
    ...
    if (channelID) {
      const cursor = getCursorFromIndex(formattedText, result.index)
      cm.replaceRange(`<#${channelID}>`, cursor, {
        line: cursor.line,
        ch: cursor.ch + match.length
      })
    }
