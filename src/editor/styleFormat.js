...
function includeBeforeAfterText(
  cm,
  fixedSelectionRange,
  lengthBefore,
  lengthAfter
...
function insideFormatting(
  ...
  const containedText = includeBeforeAfterText(
    cm,
    fixedSelectionRange,
    lengthBefore,
    lengthAfter
  ...
  return (
    containedText.startsWith(formattingBefore) &&
    containedText.endsWith(formattingAfter)
  )
}
  ...
  if (
    ...
    cm.setSelection(
      ...
      },
      {
        line: selectionRange.end.line,
        ch:
          selectionRange.end.ch -
          (isMultiLineSelection(selectionRange) ? 0 : lengthBefore)
      }
    )
  } else {
    // add formatting
    cm.replaceSelection(
      textBeforeSelection + selection + textAfterSelection,
      selection
    )
...
function updateSingleLineStyleFormat(cm, lineStartText, lineEndText = '') {
  ...
  if (
    selectedLineText.startsWith(lineStartText) &&
    selectedLineText.endsWith(lineEndText)
  ) {
    cm.replaceRange(
      selectedLineText.substring(
        lengthStartText,
        selectedLineText.length - lengthEndText
      ),
      {
    ...
    cm.replaceRange(
      lineStartText + selectedLineText + lineEndText,
      {
        line: selection.start.line,
        ch: 0
      },
    ...
    cm.setSelection({
      line: selection.start.line,
      ch:
        selectedLineText.length +
        lengthStartText +
        (selectedLineText === '' ? 0 : lengthEndText)
    })
  }
}
